...
def chaxun():
    ...
    for i in icbcbank:
        if icbcbank[i]["account"] == account:
            ...
            if icbcbank[i]["password"] == password:
...
--------------------------------------------
                                    '''
                print(info % (icbcbank[i]["name"],icbcbank[i]["account"],
                              icbcbank[i]["ID"],icbcbank[i]["country"],
                              icbcbank[i]["province"],icbcbank[i]["street"],
                              icbcbank[i]["House"],icbcbank[i]["money"],
                              icbcbank[i]["type"],bankname))
                return
            else:
                print("密码错误！")
                break
...
def cunqian():
    ...
    for i in icbcbank:#把银行里的数据依次取出来，赋给i
        if icbcbank[i]["account"] == account:
            money = input("请输入存款金额：")
            if money.isdigit():
                money = int(money)
                icbcbank[i]["money"] += money
                print("您的当前余额为：￥",icbcbank[i]["money"])
                return
            else:
                print("请输入数字！")
                break
    ...
    for i in icbcbank:#把银行里的数据依次取出来，赋给i
        if icbcbank[i]["account"] == account:
            ...
            if icbcbank[i]["password"] == password:
                ...
                if money.isdigit():
                    money = int(money)
                    if icbcbank[i]["money"] >= money:
                        icbcbank[i]["money"] -= money
                        print("取款成功！您的当前余额为：￥",icbcbank[i]["money"])
                        return
                    else:
                        print("余额不足！")
                        break
...
def zhuangzhang():
    ...
    for i in icbcbank:
        if icbcbank[i]["account"] == account1:
            password = input("请输入密码：")
            if icbcbank[i]["password"] == password:
                bianhao = input("请输入序号：\n1.普通转账\n2.跨行转账\n")
                if bianhao == "1":
                    ...
                    for j in icbcbank:
                        if icbcbank[j]["account"] == accounts:
                            money = input("请输入转账金额：")
                            if money.isdigit():
                                money = int(money)
                                if icbcbank[i]["money"] >= money:
                                    if icbcbank[i]["type"] == "普通卡":
                                        if money <= 20000:
                                            icbcbank[i]["money"] -= money
                                            icbcbank[j]["money"] += money
                                            print("转账成功！转出账户余额为：￥",icbcbank[i]["money"])
                                            print("到账成功！转入账户余额为：￥", icbcbank[j]["money"])
                                            return
                                        else:
                                            print("您的单次最大转账额度为2万元，请升级为白金卡，单次转账额度为5万元！")
                                            return
                                    elif icbcbank[i]["type"] == "白金卡":
                                        if money <= 50000:
                                            icbcbank[i]["money"] -= money
                                            icbcbank[j]["money"] += money
                                            print("转账成功！转出账户余额为：￥", icbcbank[i]["money"])
                            ...
                            if money.isdigit():
                                ...
                                if icbcbank[i]["money"] >= money:
                                    if icbcbank[i]["type"] == "普通卡":
                                        if money <= 20000:
                                            icbcbank[i]["money"] -= money
                                            abcbank[k]["money"] += money * 0.998
                                            print("转账成功！转出账户余额为：￥",icbcbank[i]["money"])
                                            print("到账成功！转入账户余额为：￥", abcbank[k]["money"])
                                            return
                                        else:
                                            print("您的单次最大转账额度为2万元，请升级为白金卡，单次转账额度为5万元！")
                                            return
